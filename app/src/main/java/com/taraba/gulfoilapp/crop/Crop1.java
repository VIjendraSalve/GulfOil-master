package com.taraba.gulfoilapp.crop;
...
public class Crop1 {
    ...
    public static final int REQUEST_CROP_CAMERA = 6700;
    public static final int REQUEST_PICK = 9162;
    public static final int REQUEST_PICK_CAMERA = 9100;
     ...
     */
    public static Crop1 of(Uri source, Uri destination) {
        return new Crop1(source, destination);
    }

    private Crop1(Uri source, Uri destination) {
     ...
     * @param y Aspect Y
     */
    public Crop1 withAspect(int x, int y) {
     ...
     */
    public Crop1 withMaxSize(int width, int height) {
     ...
     * @param fragment Fragment to receive result
     */
    public void start(Context context, androidx.fragment.app.Fragment fragment, String req) {
        if (req.equals("Camera"))
            start(context, fragment, REQUEST_CROP_CAMERA);
        else if (req.equals("Gallery"))
            start(context, fragment, REQUEST_CROP);
    }
     ...
     * @param fragment Fragment to receive result
     */
    public static void pickImage(Context context, androidx.fragment.app.Fragment fragment, String Request_code) {

        if (Request_code.equals("Camera"))
            pickImage(context, fragment, REQUEST_PICK_CAMERA);
        else if (Request_code.equals("Gallery"))
            pickImage(context, fragment, REQUEST_PICK);
    }
    ...
    public static void pickImage(Activity activity, int requestCode) {
        try {
            activity.startActivityForResult(getImagePicker(requestCode), requestCode);
    ...
    public static void pickImage(Context context, Fragment fragment, int requestCode) {
        try {
            fragment.startActivityForResult(getImagePicker(requestCode), requestCode);
        } catch (ActivityNotFoundException e) {
    ...
    private static Intent getImagePicker(int request_code) {

        if (request_code == REQUEST_PICK)
            return new Intent(Intent.ACTION_GET_CONTENT).setType("image/*");
        else if (request_code == REQUEST_PICK_CAMERA)
